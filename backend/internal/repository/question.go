...
func (r *QuestionRepo) FindByQuizID(ctx context.Context, quizID string) ([]model.Question, error) {
	...
	err := r.db.SelectContext(ctx, &questions, `SELECT id, quiz_id, type, text, image_url, code_snippet, score, sort_order FROM questions WHERE quiz_id=$1 ORDER BY sort_order`, quizID)
	return questions, err
}

func (r *QuestionRepo) FindByID(ctx context.Context, id string) (*model.Question, error) {
	var q model.Question
	err := r.db.GetContext(ctx, &q, `SELECT id, quiz_id, type, text, image_url, code_snippet, score, sort_order FROM questions WHERE id=$1`, id)
	if err != nil {
		return nil, err
	}
	return &q, nil
...
func (r *QuestionRepo) Create(ctx context.Context, q *model.Question) error {
	query := `INSERT INTO questions (quiz_id, type, text, image_url, code_snippet, score, sort_order) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`
	return r.db.QueryRowxContext(ctx, query, q.QuizID, q.Type, q.Text, q.ImageURL, q.CodeSnippet, q.Score, q.SortOrder).Scan(&q.ID)
}
...
func (r *QuestionRepo) update(ctx context.Context, execer sqlx.ExtContext, q *model.Question) error {
	query := `UPDATE questions SET type=$1, text=$2, image_url=$3, code_snippet=$4, score=$5, sort_order=$6 WHERE id=$7`
	_, err := execer.ExecContext(ctx, query, q.Type, q.Text, q.ImageURL, q.CodeSnippet, q.Score, q.SortOrder, q.ID)
	return err
}
